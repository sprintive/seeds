...
namespace Drupal\seeds\Command;
 ...
 * the current Seeds codebase to stdout.
 */
class VersionCommand extends Command {
  ...
  public function __construct($app_root) {
    parent::__construct('seeds:version');
  ...
  protected function execute(InputInterface $input, OutputInterface $output) {
    ...
    foreach ($finder as $info_file) {
      ...
      if ($info['distribution']['name'] === 'Seeds' && isset($info['version'])) {
        break;
      }
    }
    if (!isset($info)) {
      throw new \Exception('Seeds info file not found.');
    }
  ...
  /**
   * Converts a Seeds release version to a semantic version number according
   * to Seeds's VERSIONS.md file. Examples:
   * - 8.x-1.23 => 1.2.3
   * - 8.x-1.203 => 1.2.3
   * - 8.x-1.230 => 1.2.30
